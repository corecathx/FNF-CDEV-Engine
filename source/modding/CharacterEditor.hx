...
class CharacterEditor extends states.MusicBeatState
{
	...
	override function create()
	{
		...
		{
			if (isPp1)
				characterToAdd = states.PlayState.SONG.player1;
			else
				characterToAdd = states.PlayState.SONG.player2;
		}
	...
	function reloadCharSprite()
	{
		...
		if (cdev.CDevConfig.utils.fileIsExists('images/' + char.imgFile + '.txt', TEXT))
		...
		else
		{
			for (i in 0...inputTexts.length)
			{
				if (inputTexts[i].hasFocus)
				{
					...
					{
						inputTexts[i].text = cdev.CDevConfig.utils.pasteFunction(inputTexts[i].text);
