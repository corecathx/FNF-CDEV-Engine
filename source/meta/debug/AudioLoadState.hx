...
class AudioLoadState extends MusicBeatState {
    ...
    function processDropFile(data:String){
        ...
        new FlxTimer().start(2, (_)->{
            ...
            var sndObj:Sound = null;
            if (!FlxG.keys.pressed.SPACE){
                sndObj = Sound.fromAudioBuffer(
                    (data.endsWith(".ogg") ? AudioBuffer.fromVorbisFile(
                        VorbisFile.fromFile(data)
                    ) : AudioBuffer.fromFile(data))
                );
                FlxG.sound.playMusic(sndObj);
            } else{
            ...
            changeDisplayText(
                ...
                'Sample Rate : ${FlxG.keys.pressed.SPACE ? "Undefined" : '${sndObj.sampleRate}'}\n' +
