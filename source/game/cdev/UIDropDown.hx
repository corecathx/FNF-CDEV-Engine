...
//basically flxuidropdown, but supports scrolling using mouse...
/**
 * @author larsiusprime
 */
class UIDropDown extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public function new(X:Float = 0, Y:Float = 0, DataList:Array<StrNameLabel>, ?Callback:String->Void, ?Header:FlxUIDropDownHeader,
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->UIDropDown->Void)
	{
		super(X, Y);
		callback = Callback;
		header = Header;
	...
	private function updateButtonPositions():Void
	{
		...
		for (i in curScroll...list.length)
		{
			var button:FlxUIButton = list[i];
			if (button != null)
			{
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (dropPanel.visible)
		{
			if (list.length > 1 && canScroll)
			{
				...
				{
					curScroll++;
					if (curScroll >= list.length)
						curScroll = list.length - 1;
					updateButtonPositions();
				}
			}
