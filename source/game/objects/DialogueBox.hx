...
class DialogueBox extends FlxSpriteGroup
{
	...
	var bgFadeColor:FlxColor = 0xFFB3DFd8;

	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		super();

		bgFade = new FlxSprite(-200, -200).makeGraphic(Std.int(FlxG.width * 1.3), Std.int(FlxG.height * 1.3), bgFadeColor);
		...
		switch (PlayState.SONG.info.name.toLowerCase())
		{
			...
			case 'thorns':
				bgFadeColor = FlxColor.fromRGB(150,0,0);
				hasDialog = true;
		...
		dropText.font = FunkinFonts.PIXEL;
		dropText.color = 0xFFD89494;
		add(dropText);

		swagDialogue = new FlxTypeText(240, 500, Std.int(FlxG.width * 0.6), "", 32);
		swagDialogue.font = FunkinFonts.PIXEL;
	...
	var lerpAlpha:Bool = false;
	var spriteAlpha:Float = 1;
	...
	override function update(elapsed:Float)
	{
		...
		if (dropText.text.length >= dialogueList[0].length-1)
		{
			finished = true;
		} else{
			finished = false;
		...
		if (lerpAlpha){
			handSelect.alpha = FlxMath.lerp(1, handSelect.alpha, 0.9);
			handSelect.y = FlxMath.lerp(FlxG.height * 0.9, handSelect.y, 0.9);
		}else{
			handSelect.alpha = 0;
			handSelect.y = (FlxG.height * 0.9)-20;
		}
