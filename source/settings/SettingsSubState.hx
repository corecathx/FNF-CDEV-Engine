...
import settings.data.SettingsProperties.SettingsType;
import flixel.FlxG;
...
import settings.data.SettingsProperties.SettingsCategory;
...
class SettingsSubState extends MusicBeatSubstate
{
		...
		switch (curSet.type)
		{
			case 0: // bool
				if (controls.ACCEPT)
				{
					...
					CDevConfig.setData(curSet.name, !CDevConfig.getData(curSet.name));
			...
			case 1: // int
				...
				{
					CDevConfig.setData(curSet.name, CDevConfig.getData(curSet.name) + daValueToAdd);
					updateText(theText.ID);
				}
				curSet.value_name[0] = CDevConfig.getData(curSet.name);
			case 2: // float
	...
	function updateText(set:Int)
	{
		...
		{
			case 0:
				if (CDevConfig.getData(theCat.settings[set].name))
