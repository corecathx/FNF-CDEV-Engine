...
class OffsetTest extends MusicBeatState
{
	...
	{
		bg.alpha = FlxMath.lerp(0.7, bg.alpha, CDevConfig.utils.bound(1 - (elapsed * 3), 0, 1));
		...
		noteGrp.forEachAlive(function(daNoet:Note)
		{
			if (daNoet.mustPress)
			{
				if (daNoet.strumTime > Conductor.songPosition - (Conductor.safeZoneOffset * 1.5)
					&& daNoet.strumTime < Conductor.songPosition + (Conductor.safeZoneOffset * 0.5))
