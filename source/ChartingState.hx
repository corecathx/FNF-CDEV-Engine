...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	{
		var bgShit:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menuDesat'));
		bgShit.alpha = 0.5;
		bgShit.color.setRGB(FlxG.random.int(50, 205), FlxG.random.int(50, 205), FlxG.random.int(50, 205));
		...
		leftIcon.setPosition(0 + 15, -110);
		rightIcon.setPosition((gridBG.width / 2) + 15, -110);

		leftIcon.setGraphicSize(Std.int(100));
		rightIcon.setGraphicSize(Std.int(100));
		...
		bpmTxt = new FlxText(1000 - 50, 50, 1000, "", 16);
		...
		\n[W], [S] / Mouse Wheel - Scroll the strum line position.
		\n[Q] / [E] - Change the selected note sustain length.
		...
		{
			var tipsTxt:FlxText = new FlxText(UI_box.x + 10, UI_box.y + UI_box.height + 8, 0, splittedTextArray[i], 16);
			tipsTxt.y += i * 12;
	...
	function addSongUI():Void
	{
		...
		var stageListShit:Array<String> = CoolUtil.coolTextFile(Paths.txt('stageList'));
		...
		var stageDropDown = new FlxUIDropDownMenu(140, 170, FlxUIDropDownMenu.makeStrIdLabelArray(stageListShit, true), function(stageList:String)
		{
			_song.stage = stageListShit[Std.parseInt(stageList)];
		});

		var gfDropDown = new FlxUIDropDownMenu(10, 170, FlxUIDropDownMenu.makeStrIdLabelArray(gfList, true), function(daGFList:String)
		{
			_song.gfVersion = gfList[Std.parseInt(daGFList)];
		});
		...
		var oftText:FlxText = new FlxText(stepperOffset.x + 60, stepperOffset.y, FlxG.width, "Chart Offset", 8);
	...
	function addSectionUI():Void
	{
		...
		check_altAnim = new FlxUICheckBox(10, 200, null, null, "Alternate Animation", 100);
	...
	function addNoteUI():Void
	{
		...
		var applyLength:FlxButton = new FlxButton(100, 30, 'Apply');
		...
		curRenderedNotes.forEach(function(note:Note)
		{
			if (FlxG.sound.music.playing)
			{
				FlxG.overlap(strumLine, note, function(_, _)
				{
					...
					{
						claps.push(note);
						FlxG.sound.play(Paths.sound('clapsfx'));
					}
				});
			}
		});
	...
	{
		return FlxMath.remapToRange(strumTime + _song.offset, 0, 16 * Conductor.stepCrochet, gridBG.y + _song.offset, gridBG.y + gridBG.height);
	}
