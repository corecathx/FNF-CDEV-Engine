...
class Signal {
	...
	**/
	public inline static function mean(y:Array<Float>) : Float
		return sum(y) / y.length;

	/**
		Returns the global maximum.
	**/
	public inline static function max(y:Array<Float>) : Float
		return y.fold(Math.max, y[0]);

	/**
		Returns the global maximum's index.
	**/
	public inline static function maxi(y:Array<Float>) : Int
		return y.foldi((yi, m, i) -> yi > y[m] ? i : m, 0);

	/**
		Returns the global minimum.
	**/
	public inline static function min(y:Array<Float>) : Float
		return y.fold(Math.min, y[0]);

	/**
		Returns the global minimum's index.
	**/
	public inline static function mini(y:Array<Float>) : Int
		return y.foldi((yi, m, i) -> yi < y[m] ? i : m, 0);
}
