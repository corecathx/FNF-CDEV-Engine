...
class FlxActionInputDigitalKeyboard extends FlxActionInputDigital
{
	...
	public function new(Key:FlxKey, Trigger:FlxInputState)
	{
        ...
        FlxG.stage.addEventListener(KeyboardEvent.KEY_UP, getInputRelease);
	}
        ...
        if (triggerStatus) {
            switch (trigger)
            {
                ...
                case PRESSED: 
                    val = true;
                case RELEASED: 
                    val = FlxG.keys.checkStatus(inputID, RELEASED) || FlxG.keys.checkStatus(inputID, JUST_RELEASED);
                case JUST_PRESSED: 
                    val = FlxG.keys.checkStatus(inputID, JUST_PRESSED);
                case JUST_RELEASED: 
                    val = FlxG.keys.checkStatus(inputID, JUST_RELEASED);
                #else
                default: 
                    val = false;
                #end
    ...
    function getInput(e:KeyboardEvent){
        var code:FlxKey = e.keyCode;
        if (code == inputID){
            triggerStatus = switch (trigger)
            {
                #if !FLX_NO_KEYBOARD
                case PRESSED: 
                     FlxG.keys.checkStatus(inputID, PRESSED) || FlxG.keys.checkStatus(inputID, JUST_PRESSED);
    ...
    function getInputRelease(e:KeyboardEvent){
        var code:FlxKey = e.keyCode;
        if (code == inputID){
            triggerStatus = false;
        }
